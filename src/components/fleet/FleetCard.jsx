...
const FleetCard = ({ vehicle, isLoading = false, index = 0 }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -12 }}
      className="glass-card-light rounded-2xl overflow-hidden smooth-transition cursor-pointer hover:glow-effect"
    >
      {/* Image */}
      ...
      <div className="p-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              variant="outline"
              className="rounded-xl border-2 border-primary hover:bg-primary hover:text-white transition"
              onClick={() => navigate(`/vehicle/${vehicle.id}`)}
            >
              View Details
            </Button>
            <Button
              className="rounded-xl bg-[#56B24C] hover:bg-green-700 text-white transition"
              onClick={() => navigate(`/individual-booking?vehicle=${vehicle.id}`)}
            >
              Book Now
            </Button>
          </div>
